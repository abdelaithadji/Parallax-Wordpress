 ...
 */
function twentysixty_customize_register( $wp_customize ) {
	...
	$wp_customize->add_setting( 'color_scheme', array(
		'default'           => 'default',
		'sanitize_callback' => 'twentysixty_sanitize_color_scheme',
		'transport'         => 'postMessage',
	) );

	$wp_customize->add_control( 'color_scheme', array(
		'label'    => __( 'Base Color Scheme', 'twentysixty' ),
		'section'  => 'colors',
		'type'     => 'select',
		'choices'  => twentysixty_get_color_scheme_choices(),
		'priority' => 1,
	) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'sidebar_textcolor', array(
		'label'       => __( 'Header and Sidebar Text Color', 'twentysixty' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'twentysixty' ),
		'section'     => 'colors',
	) ) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'header_background_color', array(
		'label'       => __( 'Header and Sidebar Background Color', 'twentysixty' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'twentysixty' ),
		'section'     => 'colors',
	) ) );

	// Add an additional description to the header image section.
	$wp_customize->get_section( 'header_image' )->description = __( 'Applied to the header on small screens and the sidebar on wide screens.', 'twentysixty' );
}
add_action( 'customize_register', 'twentysixty_customize_register', 11 );

/**
 * Register color schemes for Twenty Fifteen.
 *
 * Can be filtered with {@see 'twentysixty_color_schemes'}.
 *
 ...
 */
function twentysixty_get_color_schemes() {
	return apply_filters( 'twentysixty_color_schemes', array(
		'default' => array(
			'label'  => __( 'Default', 'twentysixty' ),
			'colors' => array(
				'#f1f1f1',
				'#ffffff',
				'#ffffff',
...
if ( ! function_exists( 'twentysixty_get_color_scheme' ) ) :
/**
 ...
 */
function twentysixty_get_color_scheme() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	$color_schemes       = twentysixty_get_color_schemes();
...
endif; // twentysixty_get_color_scheme

if ( ! function_exists( 'twentysixty_get_color_scheme_choices' ) ) :
/**
 ...
 */
function twentysixty_get_color_scheme_choices() {
	$color_schemes                = twentysixty_get_color_schemes();
...
endif; // twentysixty_get_color_scheme_choices

if ( ! function_exists( 'twentysixty_sanitize_color_scheme' ) ) :
/**
 ...
 */
function twentysixty_sanitize_color_scheme( $value ) {
	$color_schemes = twentysixty_get_color_scheme_choices();
...
endif; // twentysixty_sanitize_color_scheme
 ...
 */
function twentysixty_color_scheme_css() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	...
	$color_textcolor_rgb         = twentysixty_hex2rgb( $color_scheme[3] );
	$color_sidebar_textcolor_rgb = twentysixty_hex2rgb( $color_scheme[4] );
	$colors = array(
	...
	$color_scheme_css = twentysixty_get_color_scheme_css( $colors );

	wp_add_inline_style( 'twentysixty-style', $color_scheme_css );
}
add_action( 'wp_enqueue_scripts', 'twentysixty_color_scheme_css' );
 ...
 * @since Twenty Fifteen 1.0
 */
function twentysixty_customize_control_js() {
	...
	wp_localize_script( 'color-scheme-control', 'colorScheme', twentysixty_get_color_schemes() );
}
add_action( 'customize_controls_enqueue_scripts', 'twentysixty_customize_control_js' );
 ...
 * @since Twenty Fifteen 1.0
 */
function twentysixty_customize_preview_js() {
...
add_action( 'customize_preview_init', 'twentysixty_customize_preview_js' );
 ...
 */
function twentysixty_get_color_scheme_css( $colors ) {
 ...
 * @since Twenty Fifteen 1.0
 */
function twentysixty_color_scheme_css_template() {
	$colors = array(
	...
	?>
	<script type="text/html" id="tmpl-twentysixty-color-scheme">
		<?php echo twentysixty_get_color_scheme_css( $colors ); ?>
	</script>
	<?php
}
add_action( 'customize_controls_print_footer_scripts', 'twentysixty_color_scheme_css_template' );
